...
class Bria4BAdapt:
    ...
    def train(
        ...
        def preprocess_train(record):
            example = {}
            image = train_transforms(record[dataset_config.image_column].convert("RGB"))

            dense_caption_column = f"dense_{dataset_config.caption_column}"

            if dense_caption_column in record:
                caption = record[dense_caption_column]
            ...
            if (
                ...
                caption = "A photo of a " + imagenet_labels[caption]

            example = {"pixel_values": image, "caption": caption}
            return example

        with accelerator.main_process_first():
            train_dataset = dataset.map(preprocess_train_webdataset)
